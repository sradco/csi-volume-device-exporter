...
1. Check the overall multipath state on the affected node:
   ...
   kubectl debug node/$NODE -it --image=registry.access.redhat.com/ubi9/ubi-minimal \
     -- chroot /host multipath -ll $DEVICE
   ```
...
2. Identify which physical paths are down:
   ...
   ```bash
   kubectl debug node/$NODE -it --image=registry.access.redhat.com/ubi9/ubi-minimal \
     -- chroot /host multipathd show paths format "%d %t %T %s"
   ```
...
4. Check the host HBA or NIC state for the failing path:
   ...
   ```bash
   kubectl debug node/$NODE -it --image=registry.access.redhat.com/ubi9/ubi-minimal \
     -- chroot /host dmesg | grep -i "scsi\|fc\|iscsi\|nvme" | tail -50
   ```
...
- Reinstate the path manually if `multipathd` has not already done so:
  ...
  ```bash
  kubectl debug node/$NODE -it --image=registry.access.redhat.com/ubi9/ubi-minimal \
    -- chroot /host multipathd reinstate path <path_device>
  ```
...
- Restart `multipathd` on the node (only if no active I/O is in critical section):
  ...
  ```bash
  kubectl debug node/$NODE -it --image=registry.access.redhat.com/ubi9/ubi-minimal \
    -- chroot /host systemctl restart multipathd
  ```
