...
import (
	...
	"github.com/sradco/csi-volume-device-exporter/pkg/discovery"
)
...
func TestObserveDiscoveryDuration_RecordsHistogram(t *testing.T) {
	...
	expected := `
# HELP csi_volume_device_exporter_discovery_duration_seconds Duration of volume discovery by discoverer.
# TYPE csi_volume_device_exporter_discovery_duration_seconds histogram
`
	gathered, err := m.registry.Gather()
	if err != nil {
		t.Fatalf("gather: %v", err)
