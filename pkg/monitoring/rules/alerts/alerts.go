...
const (
	...
	// defaultRunbookURLTemplate is used when RUNBOOK_URL_TEMPLATE is not set.
	// TODO: update to the permanent repo URL once the project is moved out of the
	// temporary sradco fork. Set RUNBOOK_URL_TEMPLATE at deploy time in the
	// meantime to point to wherever the runbooks are actually published.
	defaultRunbookURLTemplate = "https://github.com/sradco/csi-volume-device-exporter/blob/main/docs/runbooks/%s.md"
)
...
// storagePathAlerts covers multipath storage path health for CSI volumes.
func storagePathAlerts() []Alert {
	return []Alert{
		{
			Name: "CSIVolumeMultipathDegraded",
			// Three-way join: PV → CSI device → multipath device → faulty paths.
			//
			// 1. kube_persistentvolume_info → csi_volume_node_device_info
			//    maps PV to the kernel block device (e.g. dm-5) and node.
			//
			// 2. csi_volume_node_device_info → node_dmmultipath_device_info
			//    maps the sysfs name (dm-5) to the multipath device name (mpathA).
			//    Uses label_replace to rename device→sysfs_name so the join key
			//    matches device_info's sysfs_name label.
			//
			// 3. The resulting multipath "device" label (mpathA) is joined with
			//    node_dmmultipath_path_state to detect non-active paths. The
			//    path_state carries the raw SCSI device state (running, offline,
			//    blocked, transport-offline, dead, …).
			Expr: `(
  label_replace(
    label_replace(
