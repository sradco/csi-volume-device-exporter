...
// kubelet pod directory tree.
//
// Strategy: walk /var/lib/kubelet/pods/*/volumes/kubernetes.io~csi/*/ and for
// each directory:
...
//
// Currently handles VolumeMode=Filesystem only. Block volumes (VolumeMode=Block)
// use a different kubelet path layout and require future work (see resolveVolumeDevice).
//
...
//
// NOTE: Block volumes (VolumeMode=Block) are NOT discovered by this walk.
// Kubelet places them under pods/<uid>/volumeDevices/ with vol_data.json at the
// plugin staging path (plugins/kubernetes.io/csi/volumeDevices/<specName>/data/).
// Block volume support requires a separate walk of volumeDevices/ + reading
// metadata from the plugin staging path.
