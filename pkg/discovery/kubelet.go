...
func (d *KubeletDiscoverer) Discover(ctx context.Context) ([]VolumeDevice, error) {
	...
	// Return ctx.Err() only if we were interrupted mid-flight; once all three
	// sub-discoverers have completed we consider the result valid.
...
//
// The kubelet CSI staging tree has two levels of directories:
//
//	plugins/kubernetes.io/csi/<driver-name>/<volume-hash>/vol_data.json
//
// The first-level entries are driver names; the second-level entries are
// per-volume hash directories that each contain vol_data.json.
//
// Returns the index and the number of individual vol_data.json files that failed to parse.
func (d *KubeletDiscoverer) buildVolDataIndex() (*volDataIndex, int) {
	...
	for _, driverEntry := range driverEntries {
		...
		for _, volEntry := range volumeEntries {
			...
			// Key is "<driver-name>/<volume-hash>" so discoverFilesystemVolumes
			// can reconstruct the full globalmount path correctly.
...
func (d *KubeletDiscoverer) discoverFilesystemVolumes(ctx context.Context, mounts []*mountinfo.Info, idx *volDataIndex) []VolumeDevice {
	...
	for dirName, vd := range idx.byDir {
		...
		// Use FindMountUnder to handle drivers (e.g. Ceph RBD) that mount the
		// device under a subdirectory of globalmount rather than at globalmount itself.
...
func (d *KubeletDiscoverer) resolveDevice(major, minor int) (string, error) {
	...
	if strings.HasPrefix(device, "dm-") {
		// Walk the DM slave chain only for LUKS-over-something stacks.
		// A plain multipath dm-X device is the correct device to report
		// (it matches what node_dmmultipath_path_state tracks) so we stop there.
