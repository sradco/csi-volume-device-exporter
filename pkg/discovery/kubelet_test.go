...
func TestKubeletDiscoverer_FilesystemVolume(t *testing.T) {
	...
	// Kubelet staging tree: plugins/kubernetes.io/csi/<driver>/<volume-hash>/
...
// TestKubeletDiscoverer_SeparateContainerAndHostPaths verifies that file reads
// use containerKubeletRoot while mountinfo matching uses hostKubeletRoot.
// This is the critical namespace split: if either root is swapped the
// mountpoint lookup will fail and results will be empty.
func TestKubeletDiscoverer_SeparateContainerAndHostPaths(t *testing.T) {
	tmpDir := t.TempDir()

	// containerKubeletRoot: where the container reads files from.
	containerRoot := filepath.Join(tmpDir, "container-kubelet")
	// hostKubeletRoot: the actual path visible in /proc/1/mountinfo.
	...
	// Write vol_data.json under the *container* path (two-level: driver/hash).
	...
	// Write mountinfo using the *host* path (as the real host kernel would).
	...
	// Sysfs: 8:16 -> sdb (no dm, so no uuid needed).
	devBlockDir := filepath.Join(sysDir, "dev", "block")
